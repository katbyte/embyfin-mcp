...
func TestOperationDeleteLiveTvListingProviders(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteLiveTvListingProviders(t.Context(), DeleteLiveTvListingProvidersOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
