...
func TestOperationPostDlnaProfiles(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDlnaProfiles(t.Context(), DlnaProfilesDlnaProfile{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Dlna/Profiles")
