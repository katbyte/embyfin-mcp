...
func TestOperationPostItemsByIdImagesByTypeByIndexDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsByIdImagesByTypeByIndexDelete(t.Context(), "p/id", ImageTypePrimary, 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/Images/Primary/7/Delete")
