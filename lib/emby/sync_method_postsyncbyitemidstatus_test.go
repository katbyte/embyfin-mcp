...
func TestOperationPostSyncByItemIdStatus(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostSyncByItemIdStatus(t.Context(), "p/itemId", SyncedItemProgress{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Sync/p%2FitemId/Status")
