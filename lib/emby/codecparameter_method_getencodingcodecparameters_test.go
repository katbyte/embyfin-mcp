...
func TestOperationGetEncodingCodecParameters(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetEncodingCodecParameters(t.Context(), GetEncodingCodecParametersOperationOptions{
		CodecId:          "v-CodecId",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Encoding/CodecParameters")
