...
func TestOperationDeleteUsersByIdRecentlySearched(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteUsersByIdRecentlySearched(t.Context(), "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Users/p%2Fid/RecentlySearched")
