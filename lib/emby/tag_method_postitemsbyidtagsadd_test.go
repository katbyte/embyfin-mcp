...
func TestOperationPostItemsByIdTagsAdd(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsByIdTagsAdd(t.Context(), "p/id", UserLibraryAddTags{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/Tags/Add")
