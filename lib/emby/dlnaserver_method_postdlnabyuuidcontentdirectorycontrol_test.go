...
func TestOperationPostDlnaByUuIdContentdirectoryControl(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDlnaByUuIdContentdirectoryControl(t.Context(), "p/uuId", strings.NewReader("raw bytes"), "application/octet-stream")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Dlna/p%2FuuId/contentdirectory/control")
