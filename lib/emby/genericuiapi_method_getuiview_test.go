...
func TestOperationGetUIView(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetUIView(t.Context(), GetUIViewOperationOptions{
		PageId:       "v-PageId",
		ClientLocale: "v-ClientLocale",
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/UI/View")
	expectQuery(t, r, "PageId", "v-PageId")
	expectQuery(t, r, "ClientLocale", "v-ClientLocale")
