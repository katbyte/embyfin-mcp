...
// GetGameGenresOperationResponse is the result of GetGameGenres.
type GetGameGenresOperationResponse struct {
	HttpResponse *http.Response
	Model        *QueryResultBaseItemDto
}

// GetGameGenresOperationOptions holds the query and header parameters of GetGameGenres.
type GetGameGenresOperationOptions struct {
...
func (o GetGameGenresOperationOptions) ToHeaders() *client.Headers {
	out := client.Headers{}
	return &out
}

// ToQuery returns the query parameters the options set.
func (o GetGameGenresOperationOptions) ToQuery() *client.QueryParams {
...
// GetGameGenres calls GET /GameGenres. Gets all Game genres from a given item, folder, or the entire library.
func (c Client) GetGameGenres(ctx context.Context, options GetGameGenresOperationOptions) (result GetGameGenresOperationResponse, err error) {
...
// GetGameGenresCompleteResult is every result GetGameGenresComplete loaded.
type GetGameGenresCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []BaseItemDto
}

// GetGameGenresComplete calls GetGameGenres page by page from options.StartIndex until every
// result is loaded. options.Limit is the page size, client.DefaultPageSize when
// unset.
func (c Client) GetGameGenresComplete(ctx context.Context, options GetGameGenresOperationOptions) (result GetGameGenresCompleteResult, err error) {
	if options.Limit <= 0 {
		options.Limit = client.DefaultPageSize
	}
	for {
		var page GetGameGenresOperationResponse
		page, err = c.GetGameGenres(ctx, options)
