...
func TestOperationGetGameGenresByNameImagesByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetGameGenresByNameImagesByType(t.Context(), "p/name", ImageTypePrimary, GetGameGenresByNameImagesByTypeOperationOptions{
		MaxWidth:             7,
		MaxHeight:            7,
		Width:                7,
		Height:               7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/GameGenres/p%2Fname/Images/Primary")
