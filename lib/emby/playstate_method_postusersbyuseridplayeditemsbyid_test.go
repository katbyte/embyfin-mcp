...
func TestOperationPostUsersByUserIdPlayedItemsById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostUsersByUserIdPlayedItemsById(t.Context(), "p/userId", "p/id", PostUsersByUserIdPlayedItemsByIdOperationOptions{
		DatePlayed: "v-DatePlayed",
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/PlayedItems/p%2Fid")
	expectQuery(t, r, "DatePlayed", "v-DatePlayed")
