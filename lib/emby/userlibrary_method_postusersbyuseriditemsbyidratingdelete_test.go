...
func TestOperationPostUsersByUserIdItemsByIdRatingDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostUsersByUserIdItemsByIdRatingDelete(t.Context(), "p/userId", "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/Items/p%2Fid/Rating/Delete")
