...
func TestOperationGetLiveTvLiveRecordingsByIdStream(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "video/mp4", "file bytes")
	result, err := c.GetLiveTvLiveRecordingsByIdStream(t.Context(), "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/LiveRecordings/p%2Fid/stream")
