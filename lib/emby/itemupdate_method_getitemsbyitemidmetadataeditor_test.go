...
func TestOperationGetItemsByItemIdMetadataEditor(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetItemsByItemIdMetadataEditor(t.Context(), "p/itemId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2FitemId/MetadataEditor")
