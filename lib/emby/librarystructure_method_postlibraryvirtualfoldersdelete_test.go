...
func TestOperationPostLibraryVirtualFoldersDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLibraryVirtualFoldersDelete(t.Context(), LibraryRemoveVirtualFolder{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Library/VirtualFolders/Delete")
