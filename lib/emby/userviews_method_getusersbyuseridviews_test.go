...
func TestOperationGetUsersByUserIdViews(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetUsersByUserIdViews(t.Context(), "p/userId", GetUsersByUserIdViewsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Users/p%2FuserId/Views")
