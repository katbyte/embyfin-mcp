...
func TestOperationPostEncodingFfmpegOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostEncodingFfmpegOptions(t.Context(), strings.NewReader("raw bytes"), "application/octet-stream")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Encoding/FfmpegOptions")
