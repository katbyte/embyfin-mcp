...
func TestOperationGetLiveTvChannelMappings(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetLiveTvChannelMappings(t.Context(), GetLiveTvChannelMappingsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/ChannelMappings")
