...
func TestOperationPostItemsByIdSubtitlesByIndexDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsByIdSubtitlesByIndexDelete(t.Context(), "p/id", 7, PostItemsByIdSubtitlesByIndexDeleteOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/Subtitles/7/Delete")
