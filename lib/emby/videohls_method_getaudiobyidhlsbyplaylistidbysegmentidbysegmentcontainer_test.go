...
func TestOperationGetAudioByIdHlsByPlaylistIdBySegmentIdBySegmentContainer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetAudioByIdHlsByPlaylistIdBySegmentIdBySegmentContainer(t.Context(), "p/id", "p/playlistId", "p/segmentId", "p/segmentContainer")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Audio/p%2Fid/hls/p%2FplaylistId/p%2FsegmentId.p%2FsegmentContainer")
