...
func TestOperationGetUserSettingsByUserId(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetUserSettingsByUserId(t.Context(), "p/userId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/UserSettings/p%2FuserId")
