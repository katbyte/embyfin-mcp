...
func TestOperationPostLiveTvSeriesTimersById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLiveTvSeriesTimersById(t.Context(), "p/id", LiveTvSeriesTimerInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/SeriesTimers/p%2Fid")
