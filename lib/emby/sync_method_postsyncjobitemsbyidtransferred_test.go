...
func TestOperationPostSyncJobItemsByIdTransferred(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostSyncJobItemsByIdTransferred(t.Context(), "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sync/JobItems/p%2Fid/Transferred")
