...
func TestOperationPostBackupRestoreRestore(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostBackupRestoreRestore(t.Context(), MBBackupApiRestoreOptions{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/BackupRestore/Restore")
