...
func TestOperationGetVideosByIdIndexBif(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetVideosByIdIndexBif(t.Context(), "p/id", GetVideosByIdIndexBifOperationOptions{
		Width: 7,
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Videos/p%2Fid/index.bif")
