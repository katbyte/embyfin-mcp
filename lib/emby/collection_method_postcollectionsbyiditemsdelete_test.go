...
func TestOperationPostCollectionsByIdItemsDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostCollectionsByIdItemsDelete(t.Context(), "p/id", PostCollectionsByIdItemsDeleteOperationOptions{
		Ids: "v-Ids",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Collections/p%2Fid/Items/Delete")
