...
func TestOperationPostItemsByIdImagesByTypeByIndexIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsByIdImagesByTypeByIndexIndex(t.Context(), "p/id", ImageTypePrimary, 7, PostItemsByIdImagesByTypeByIndexIndexOperationOptions{
		NewIndex: new(7),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/Images/Primary/7/Index")
