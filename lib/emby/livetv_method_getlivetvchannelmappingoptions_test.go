...
func TestOperationGetLiveTvChannelMappingOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetLiveTvChannelMappingOptions(t.Context(), GetLiveTvChannelMappingOptionsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/ChannelMappingOptions")
