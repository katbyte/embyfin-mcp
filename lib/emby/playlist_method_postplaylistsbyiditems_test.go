...
func TestOperationPostPlaylistsByIdItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostPlaylistsByIdItems(t.Context(), "p/id", PostPlaylistsByIdItemsOperationOptions{
		UserId: "v-UserId",
		Ids:    "v-Ids",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Playlists/p%2Fid/Items")
