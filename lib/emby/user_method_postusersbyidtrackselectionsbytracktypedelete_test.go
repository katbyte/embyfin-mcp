...
func TestOperationPostUsersByIdTrackSelectionsByTrackTypeDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByIdTrackSelectionsByTrackTypeDelete(t.Context(), "p/id", "p/trackType")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/TrackSelections/p%2FtrackType/Delete")
