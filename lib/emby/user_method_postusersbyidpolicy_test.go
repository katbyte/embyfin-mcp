...
func TestOperationPostUsersByIdPolicy(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByIdPolicy(t.Context(), "p/id", UserPolicy{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/Policy")
