...
func TestOperationPostItemsByIdPlaybackInfo(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostItemsByIdPlaybackInfo(t.Context(), "p/id", PlaybackInfoRequest{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/PlaybackInfo")
