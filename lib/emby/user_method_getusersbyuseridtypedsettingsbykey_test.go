...
func TestOperationGetUsersByUserIdTypedSettingsByKey(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetUsersByUserIdTypedSettingsByKey(t.Context(), "p/userId", "p/key")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Users/p%2FuserId/TypedSettings/p%2Fkey")
