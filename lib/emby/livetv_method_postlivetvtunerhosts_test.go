...
func TestOperationPostLiveTvTunerHosts(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostLiveTvTunerHosts(t.Context(), LiveTvTunerHostInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/TunerHosts")
