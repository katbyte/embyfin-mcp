...
func TestOperationGetSyncJobItemsByIdAdditionalFiles(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetSyncJobItemsByIdAdditionalFiles(t.Context(), "p/id", GetSyncJobItemsByIdAdditionalFilesOperationOptions{
		Name: "v-Name",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Sync/JobItems/p%2Fid/AdditionalFiles")
