...
func TestOperationGetSessionsPlayQueue(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSessionsPlayQueue(t.Context(), GetSessionsPlayQueueOperationOptions{
		Id:       "v-Id",
		DeviceId: "v-DeviceId",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Sessions/PlayQueue")
