...
func TestOperationPostLiveTvManageChannelsByIdDisabled(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostLiveTvManageChannelsByIdDisabled(t.Context(), "p/id", ApiSetChannelDisabled{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/Manage/Channels/p%2Fid/Disabled")
