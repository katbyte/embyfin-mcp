...
func TestOperationPutLiveTvChannelMappings(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PutLiveTvChannelMappings(t.Context(), ApiSetChannelMapping{}, PutLiveTvChannelMappingsOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPut, "/LiveTv/ChannelMappings")
