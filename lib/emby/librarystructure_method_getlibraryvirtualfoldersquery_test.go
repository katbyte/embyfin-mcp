...
func TestOperationGetLibraryVirtualFoldersQuery(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetLibraryVirtualFoldersQuery(t.Context(), GetLibraryVirtualFoldersQueryOperationOptions{
		StartIndex: new(7),
		Limit:      new(7),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Library/VirtualFolders/Query")
...
func TestOperationGetLibraryVirtualFoldersQueryComplete(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetLibraryVirtualFoldersQueryOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetLibraryVirtualFoldersQueryOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetLibraryVirtualFoldersQueryOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetLibraryVirtualFoldersQueryOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "StartIndex", "{}", tc.total, tc.pages)
			result, err := c.GetLibraryVirtualFoldersQueryComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
