...
func TestOperationDeleteLiveTvChannelMappings(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteLiveTvChannelMappings(t.Context(), DeleteLiveTvChannelMappingsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/LiveTv/ChannelMappings")
