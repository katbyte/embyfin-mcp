...
func TestOperationGetSystemLogsByNameLines(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSystemLogsByNameLines(t.Context(), "p/name")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/System/Logs/p%2Fname/Lines")
