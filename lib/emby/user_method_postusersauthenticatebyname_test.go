...
func TestOperationPostUsersAuthenticateByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostUsersAuthenticateByName(t.Context(), AuthenticateUserByName{}, PostUsersAuthenticateByNameOperationOptions{
		XEmbyAuthorization: "v-XEmbyAuthorization",
	})
	if err != nil {
		t.Fatal(err)
	}
