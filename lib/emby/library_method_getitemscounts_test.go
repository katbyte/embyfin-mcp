...
func TestOperationGetItemsCounts(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetItemsCounts(t.Context(), GetItemsCountsOperationOptions{
		UserId:     "v-UserId",
		IsFavorite: new(true),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/Counts")
