...
func TestOperationGetPackagesByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetPackagesByName(t.Context(), "p/name", GetPackagesByNameOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Packages/p%2Fname")
