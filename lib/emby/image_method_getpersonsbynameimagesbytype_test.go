...
func TestOperationGetPersonsByNameImagesByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetPersonsByNameImagesByType(t.Context(), "p/name", ImageTypePrimary, GetPersonsByNameImagesByTypeOperationOptions{
		MaxWidth:             7,
		MaxHeight:            7,
		Width:                7,
		Height:               7,
