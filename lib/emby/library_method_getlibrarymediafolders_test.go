...
func TestOperationGetLibraryMediaFolders(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetLibraryMediaFolders(t.Context(), GetLibraryMediaFoldersOperationOptions{
		IsHidden: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Library/MediaFolders")
