...
func TestOperationGetUsersQueryComplete(t *testing.T) {
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetUsersQueryOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetUsersQueryOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetUsersQueryOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetUsersQueryOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "StartIndex", "{}", tc.total, tc.pages)
			result, err := c.GetUsersQueryComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
