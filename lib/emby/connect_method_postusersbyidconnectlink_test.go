...
func TestOperationPostUsersByIdConnectLink(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostUsersByIdConnectLink(t.Context(), "p/id", PostUsersByIdConnectLinkOperationOptions{
		ConnectUsername: "v-ConnectUsername",
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/Connect/Link")
	expectQuery(t, r, "ConnectUsername", "v-ConnectUsername")
