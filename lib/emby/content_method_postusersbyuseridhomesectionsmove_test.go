...
func TestOperationPostUsersByUserIdHomeSectionsMove(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByUserIdHomeSectionsMove(t.Context(), "p/userId", UserLibraryMoveHomeSections{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/HomeSections/Move")
