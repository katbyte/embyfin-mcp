...
func TestOperationGetGameGenresByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetGameGenresByName(t.Context(), "p/name", GetGameGenresByNameOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/GameGenres/p%2Fname")
