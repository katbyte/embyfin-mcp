...
func TestOperationPostSessionsByIdViewing(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostSessionsByIdViewing(t.Context(), "p/id", PostSessionsByIdViewingOperationOptions{
		ItemType: "v-ItemType",
		ItemId:   "v-ItemId",
		ItemName: "v-ItemName",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/p%2Fid/Viewing")
	expectQuery(t, r, "ItemType", "v-ItemType")
