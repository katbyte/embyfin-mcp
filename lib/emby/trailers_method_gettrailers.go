...
// GetTrailersOperationResponse is the result of GetTrailers.
type GetTrailersOperationResponse struct {
	HttpResponse *http.Response
	Model        *QueryResultBaseItemDto
}

// GetTrailersOperationOptions holds the query and header parameters of GetTrailers.
type GetTrailersOperationOptions struct {
...
func (o GetTrailersOperationOptions) ToHeaders() *client.Headers {
	out := client.Headers{}
	return &out
}

// ToQuery returns the query parameters the options set.
func (o GetTrailersOperationOptions) ToQuery() *client.QueryParams {
...
// GetTrailers calls GET /Trailers. Finds movies and trailers similar to a given trailer.
func (c Client) GetTrailers(ctx context.Context, options GetTrailersOperationOptions) (result GetTrailersOperationResponse, err error) {
...
// GetTrailersCompleteResult is every result GetTrailersComplete loaded.
type GetTrailersCompleteResult struct {
	LatestHttpResponse *http.Response
	Items              []BaseItemDto
}

// GetTrailersComplete calls GetTrailers page by page from options.StartIndex until every
// result is loaded. options.Limit is the page size, client.DefaultPageSize when
// unset.
func (c Client) GetTrailersComplete(ctx context.Context, options GetTrailersOperationOptions) (result GetTrailersCompleteResult, err error) {
	if options.Limit <= 0 {
		options.Limit = client.DefaultPageSize
	}
	for {
		var page GetTrailersOperationResponse
		page, err = c.GetTrailers(ctx, options)
