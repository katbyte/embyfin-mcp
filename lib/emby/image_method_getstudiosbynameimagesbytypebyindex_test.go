...
func TestOperationGetStudiosByNameImagesByTypeByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetStudiosByNameImagesByTypeByIndex(t.Context(), "p/name", ImageTypePrimary, 7, GetStudiosByNameImagesByTypeByIndexOperationOptions{
		MaxWidth:             7,
		MaxHeight:            7,
		Width:                7,
		Height:               7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Studios/p%2Fname/Images/Primary/7")
