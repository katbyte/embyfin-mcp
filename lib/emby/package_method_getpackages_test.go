...
func TestOperationGetPackages(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetPackages(t.Context(), GetPackagesOperationOptions{
		PackageType:   "v-PackageType",
		TargetSystems: "v-TargetSystems",
		IsPremium:     new(true),
		IsAdult:       new(true),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Packages")
	expectQuery(t, r, "PackageType", "v-PackageType")
	expectQuery(t, r, "TargetSystems", "v-TargetSystems")
	expectQuery(t, r, "IsPremium", "true")
	expectQuery(t, r, "IsAdult", "true")
