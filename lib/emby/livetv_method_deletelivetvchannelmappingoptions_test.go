...
func TestOperationDeleteLiveTvChannelMappingOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteLiveTvChannelMappingOptions(t.Context(), DeleteLiveTvChannelMappingOptionsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/LiveTv/ChannelMappingOptions")
