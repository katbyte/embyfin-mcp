...
func TestOperationPostUsersByIdPassword(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByIdPassword(t.Context(), "p/id", UpdateUserPassword{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/Password")
