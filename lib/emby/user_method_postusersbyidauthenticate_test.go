...
func TestOperationPostUsersByIdAuthenticate(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostUsersByIdAuthenticate(t.Context(), "p/id", AuthenticateUser{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/Authenticate")
