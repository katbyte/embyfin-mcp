...
func TestOperationGetUsersItemAccess(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetUsersItemAccess(t.Context(), GetUsersItemAccessOperationOptions{
		IsHidden:                new(true),
		IsDisabled:              new(true),
		StartIndex:              new(7),
		Limit:                   new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Users/ItemAccess")
...
func TestOperationGetUsersItemAccessComplete(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetUsersItemAccessOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetUsersItemAccessOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetUsersItemAccessOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetUsersItemAccessOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "StartIndex", "{}", tc.total, tc.pages)
			result, err := c.GetUsersItemAccessComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
