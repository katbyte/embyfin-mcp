...
func TestOperationPostPlaylistsByIdItemsDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostPlaylistsByIdItemsDelete(t.Context(), "p/id", PostPlaylistsByIdItemsDeleteOperationOptions{
		EntryIds: "v-EntryIds",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Playlists/p%2Fid/Items/Delete")
