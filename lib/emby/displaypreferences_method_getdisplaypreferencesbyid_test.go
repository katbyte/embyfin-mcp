...
func TestOperationGetDisplayPreferencesById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetDisplayPreferencesById(t.Context(), "p/id", GetDisplayPreferencesByIdOperationOptions{
		UserId: "v-UserId",
		Client: "v-Client",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/DisplayPreferences/p%2Fid")
