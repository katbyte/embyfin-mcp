...
func TestOperationDeleteSyncByTargetIdItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteSyncByTargetIdItems(t.Context(), "p/targetId", DeleteSyncByTargetIdItemsOperationOptions{
		ItemIds: "v-ItemIds",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Sync/p%2FtargetId/Items")
