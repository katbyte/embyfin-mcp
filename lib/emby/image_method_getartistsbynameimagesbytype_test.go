...
func TestOperationGetArtistsByNameImagesByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetArtistsByNameImagesByType(t.Context(), "p/name", ImageTypePrimary, GetArtistsByNameImagesByTypeOperationOptions{
		MaxWidth:             new(7),
		MaxHeight:            new(7),
		Width:                new(7),
		Height:               new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Artists/p%2Fname/Images/Primary")
