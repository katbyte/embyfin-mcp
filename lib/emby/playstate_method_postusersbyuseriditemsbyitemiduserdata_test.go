...
func TestOperationPostUsersByUserIdItemsByItemIdUserData(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByUserIdItemsByItemIdUserData(t.Context(), "p/userId", "p/itemId", UserItemDataDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/Items/p%2FitemId/UserData")
