...
func TestOperationGetLiveTvManageChannelsComplete(t *testing.T) {
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetLiveTvManageChannelsOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetLiveTvManageChannelsOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetLiveTvManageChannelsOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetLiveTvManageChannelsOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "StartIndex", "{}", tc.total, tc.pages)
			result, err := c.GetLiveTvManageChannelsComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
