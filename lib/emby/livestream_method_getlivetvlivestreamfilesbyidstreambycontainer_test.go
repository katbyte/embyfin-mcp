...
func TestOperationGetLiveTvLiveStreamFilesByIdStreamByContainer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "video/mp4", "file bytes")
	result, err := c.GetLiveTvLiveStreamFilesByIdStreamByContainer(t.Context(), "p/id", "p/container")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
