...
func TestOperationPostItemsByIdImagesByTypeByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsByIdImagesByTypeByIndex(t.Context(), "p/id", ImageTypePrimary, 7, strings.NewReader("raw bytes"), "application/octet-stream")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Items/p%2Fid/Images/Primary/7")
