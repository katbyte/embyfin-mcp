...
func TestOperationPostAuthKeysByKeyDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostAuthKeysByKeyDelete(t.Context(), "p/key")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Auth/Keys/p%2Fkey/Delete")
