...
func TestOperationGetUsersByUserIdHomeSections(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetUsersByUserIdHomeSections(t.Context(), "p/userId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Users/p%2FuserId/HomeSections")
