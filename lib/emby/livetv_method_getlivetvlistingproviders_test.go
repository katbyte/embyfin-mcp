...
func TestOperationGetLiveTvListingProviders(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetLiveTvListingProviders(t.Context(), GetLiveTvListingProvidersOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/ListingProviders")
