...
func TestOperationPostUsersByUserIdSearchedItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByUserIdSearchedItems(t.Context(), "p/userId", UserLibraryReportItemsSearched{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/SearchedItems/")
