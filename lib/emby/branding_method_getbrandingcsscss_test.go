...
func TestOperationGetBrandingCssCss(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "text/css", "file bytes")
	result, err := c.GetBrandingCssCss(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Branding/Css.css")
