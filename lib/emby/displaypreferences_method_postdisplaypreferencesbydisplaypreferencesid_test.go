...
func TestOperationPostDisplayPreferencesByDisplayPreferencesId(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDisplayPreferencesByDisplayPreferencesId(t.Context(), "p/displayPreferencesId", DisplayPreferences{}, PostDisplayPreferencesByDisplayPreferencesIdOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
