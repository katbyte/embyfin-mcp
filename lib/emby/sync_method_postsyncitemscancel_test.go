...
func TestOperationPostSyncItemsCancel(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostSyncItemsCancel(t.Context(), PostSyncItemsCancelOperationOptions{
		ItemIds: "v-ItemIds",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sync/Items/Cancel")
