...
func TestOperationGetMusicGenresByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetMusicGenresByName(t.Context(), "p/name", GetMusicGenresByNameOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/MusicGenres/p%2Fname")
