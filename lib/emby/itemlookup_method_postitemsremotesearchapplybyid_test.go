...
func TestOperationPostItemsRemoteSearchApplyById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsRemoteSearchApplyById(t.Context(), "p/id", RemoteSearchResult{}, PostItemsRemoteSearchApplyByIdOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Items/RemoteSearch/Apply/p%2Fid")
