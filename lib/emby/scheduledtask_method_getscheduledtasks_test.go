...
func TestOperationGetScheduledTasks(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetScheduledTasks(t.Context(), GetScheduledTasksOperationOptions{
		IsHidden:  new(true),
		IsEnabled: new(true),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/ScheduledTasks")
	expectQuery(t, r, "IsHidden", "true")
	expectQuery(t, r, "IsEnabled", "true")
