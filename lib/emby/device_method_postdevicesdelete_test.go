...
func TestOperationPostDevicesDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDevicesDelete(t.Context(), PostDevicesDeleteOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Devices/Delete")
