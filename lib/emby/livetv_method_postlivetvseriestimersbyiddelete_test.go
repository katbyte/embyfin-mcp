...
func TestOperationPostLiveTvSeriesTimersByIdDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLiveTvSeriesTimersByIdDelete(t.Context(), "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/SeriesTimers/p%2Fid/Delete")
