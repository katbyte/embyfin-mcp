...
func TestOperationPostPlaylistsByIdItemsByItemIdMoveByNewIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostPlaylistsByIdItemsByItemIdMoveByNewIndex(t.Context(), "p/id", 7, 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Playlists/p%2Fid/Items/7/Move/7")
