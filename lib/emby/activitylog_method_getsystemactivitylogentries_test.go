...
func TestOperationGetSystemActivityLogEntries(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSystemActivityLogEntries(t.Context(), GetSystemActivityLogEntriesOperationOptions{
		StartIndex: new(7),
		Limit:      new(7),
		MinDate:    "v-MinDate",
	})
...
func TestOperationGetSystemActivityLogEntriesComplete(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetSystemActivityLogEntriesOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetSystemActivityLogEntriesOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetSystemActivityLogEntriesOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetSystemActivityLogEntriesOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "StartIndex", "{}", tc.total, tc.pages)
			result, err := c.GetSystemActivityLogEntriesComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
