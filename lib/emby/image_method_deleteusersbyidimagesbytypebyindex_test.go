...
func TestOperationDeleteUsersByIdImagesByTypeByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteUsersByIdImagesByTypeByIndex(t.Context(), "p/id", ImageTypePrimary, 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Users/p%2Fid/Images/Primary/7")
