...
func TestOperationPostLiveTvTimersById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLiveTvTimersById(t.Context(), "p/id", LiveTvTimerInfoDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/Timers/p%2Fid")
