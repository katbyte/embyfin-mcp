...
func TestOperationPostSessionsByIdUsersByUserIdDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostSessionsByIdUsersByUserIdDelete(t.Context(), "p/id", "p/userId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/p%2Fid/Users/p%2FuserId/Delete")
