...
func TestOperationGetItemsByIdImagesByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetItemsByIdImagesByType(t.Context(), "p/id", ImageTypePrimary, GetItemsByIdImagesByTypeOperationOptions{
		MaxWidth:             7,
		MaxHeight:            7,
		Width:                7,
		Height:               7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2Fid/Images/Primary")
