...
func TestOperationPostLibraryVirtualFoldersLibraryOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLibraryVirtualFoldersLibraryOptions(t.Context(), LibraryUpdateLibraryOptions{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
