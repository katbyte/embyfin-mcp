...
func TestOperationPostUsersByIdImagesByTypeByIndexDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByIdImagesByTypeByIndexDelete(t.Context(), "p/id", ImageTypePrimary, 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Users/p%2Fid/Images/Primary/7/Delete")
