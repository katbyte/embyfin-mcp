...
func TestOperationPostLiveTvTunerHostsDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLiveTvTunerHostsDelete(t.Context(), PostLiveTvTunerHostsDeleteOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/TunerHosts/Delete")
