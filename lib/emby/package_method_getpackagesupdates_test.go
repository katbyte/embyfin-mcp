...
func TestOperationGetPackagesUpdates(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetPackagesUpdates(t.Context(), GetPackagesUpdatesOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Packages/Updates")
