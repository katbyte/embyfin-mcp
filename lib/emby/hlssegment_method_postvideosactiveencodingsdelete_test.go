...
func TestOperationPostVideosActiveEncodingsDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostVideosActiveEncodingsDelete(t.Context(), PostVideosActiveEncodingsDeleteOperationOptions{
		DeviceId:      "v-DeviceId",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Videos/ActiveEncodings/Delete")
