...
func TestOperationPostUsersByUserIdTypedSettingsByKey(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostUsersByUserIdTypedSettingsByKey(t.Context(), "p/userId", "p/key", strings.NewReader("raw bytes"), "application/octet-stream")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/TypedSettings/p%2Fkey")
