...
func TestOperationGetDlnaByUuIdConnectionmanagerConnectionmanagerXml(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "text/xml", "file bytes")
	result, err := c.GetDlnaByUuIdConnectionmanagerConnectionmanagerXml(t.Context(), "p/uuId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Dlna/p%2FuuId/connectionmanager/connectionmanager.xml")
