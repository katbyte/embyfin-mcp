...
func TestOperationPostCollectionsByIdItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostCollectionsByIdItems(t.Context(), "p/id", PostCollectionsByIdItemsOperationOptions{
		Ids: "v-Ids",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Collections/p%2Fid/Items")
