...
func TestOperationPostLiveTvChannelMappingOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLiveTvChannelMappingOptions(t.Context(), PostLiveTvChannelMappingOptionsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/ChannelMappingOptions")
