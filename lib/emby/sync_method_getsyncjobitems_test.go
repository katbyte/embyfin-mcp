...
func TestOperationGetSyncJobItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSyncJobItems(t.Context(), GetSyncJobItemsOperationOptions{
		TargetId: "v-TargetId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Sync/JobItems")
