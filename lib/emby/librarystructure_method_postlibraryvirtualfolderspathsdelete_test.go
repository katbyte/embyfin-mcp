...
func TestOperationPostLibraryVirtualFoldersPathsDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostLibraryVirtualFoldersPathsDelete(t.Context(), LibraryRemoveMediaPath{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Library/VirtualFolders/Paths/Delete")
