...
func TestOperationPostItemsRemoteSearchGame(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.PostItemsRemoteSearchGame(t.Context(), RemoteSearchQueryGameInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Items/RemoteSearch/Game")
