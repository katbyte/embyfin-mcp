...
func TestOperationPostVideosByIdSubtitlesByIndexDelete(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostVideosByIdSubtitlesByIndexDelete(t.Context(), "p/id", 7, PostVideosByIdSubtitlesByIndexDeleteOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Videos/p%2Fid/Subtitles/7/Delete")
