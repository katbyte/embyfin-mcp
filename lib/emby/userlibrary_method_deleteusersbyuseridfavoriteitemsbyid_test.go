...
func TestOperationDeleteUsersByUserIdFavoriteItemsById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.DeleteUsersByUserIdFavoriteItemsById(t.Context(), "p/userId", "p/id")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Users/p%2FuserId/FavoriteItems/p%2Fid")
