...
func TestOperationGetItemsByIdRemoteSearchSubtitlesByLanguage(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetItemsByIdRemoteSearchSubtitlesByLanguage(t.Context(), "p/id", "p/language", GetItemsByIdRemoteSearchSubtitlesByLanguageOperationOptions{
		MediaSourceId:     "v-MediaSourceId",
		IsPerfectMatch:    new(true),
		IsForced:          new(true),
		IsHearingImpaired: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2Fid/RemoteSearch/Subtitles/p%2Flanguage")
	expectQuery(t, r, "MediaSourceId", "v-MediaSourceId")
	expectQuery(t, r, "IsPerfectMatch", "true")
	expectQuery(t, r, "IsForced", "true")
	expectQuery(t, r, "IsHearingImpaired", "true")
