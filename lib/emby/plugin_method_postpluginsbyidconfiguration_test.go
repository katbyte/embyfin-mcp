...
func TestOperationPostPluginsByIdConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostPluginsByIdConfiguration(t.Context(), "p/id", strings.NewReader("raw bytes"), "application/octet-stream")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Plugins/p%2Fid/Configuration")
