...
func TestOperationGetLiveTvLiveRecordingsByIdHlsBySegment(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetLiveTvLiveRecordingsByIdHlsBySegment(t.Context(), "p/id", "p/segment")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/LiveRecordings/p%2Fid/hls/p%2Fsegment")
