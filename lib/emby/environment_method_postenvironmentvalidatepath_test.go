...
func TestOperationPostEnvironmentValidatePath(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostEnvironmentValidatePath(t.Context(), ValidatePath{}, PostEnvironmentValidatePathOperationOptions{
		Path: "v-Path",
	})
	if err != nil {
		t.Fatal(err)
