...
func TestOperationGetGenresByNameImagesByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetGenresByNameImagesByType(t.Context(), "p/name", ImageTypePrimary, GetGenresByNameImagesByTypeOperationOptions{
		MaxWidth:             7,
		MaxHeight:            7,
		Width:                7,
		Height:               7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Genres/p%2Fname/Images/Primary")
