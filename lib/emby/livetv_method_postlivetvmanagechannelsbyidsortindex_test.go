...
func TestOperationPostLiveTvManageChannelsByIdSortIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostLiveTvManageChannelsByIdSortIndex(t.Context(), "p/id", ApiSetChannelSortIndex{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/Manage/Channels/p%2Fid/SortIndex")
