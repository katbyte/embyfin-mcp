...
func TestOperationPostBackupRestoreRestoreData(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostBackupRestoreRestoreData(t.Context(), MBBackupApiDataRestoreOptions{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/BackupRestore/RestoreData")
