...
func TestOperationDeleteItemsByIdSubtitlesByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteItemsByIdSubtitlesByIndex(t.Context(), "p/id", 7, DeleteItemsByIdSubtitlesByIndexOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Items/p%2Fid/Subtitles/7")
