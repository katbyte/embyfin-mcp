...
func TestOperationGetItemsByIdImagesByTypeByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetItemsByIdImagesByTypeByIndex(t.Context(), "p/id", ImageTypePrimary, 7, GetItemsByIdImagesByTypeByIndexOperationOptions{
		MaxWidth:             new(7),
		MaxHeight:            new(7),
		Width:                new(7),
		Height:               new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2Fid/Images/Primary/7")
