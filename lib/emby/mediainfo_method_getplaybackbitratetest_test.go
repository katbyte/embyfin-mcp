...
func TestOperationGetPlaybackBitrateTest(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetPlaybackBitrateTest(t.Context(), GetPlaybackBitrateTestOperationOptions{
		Size: 7,
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Playback/BitrateTest")
	expectQuery(t, r, "Size", "7")
