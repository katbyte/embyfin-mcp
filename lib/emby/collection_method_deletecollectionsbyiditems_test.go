...
func TestOperationDeleteCollectionsByIdItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteCollectionsByIdItems(t.Context(), "p/id", DeleteCollectionsByIdItemsOperationOptions{
		Ids: "v-Ids",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Collections/p%2Fid/Items")
