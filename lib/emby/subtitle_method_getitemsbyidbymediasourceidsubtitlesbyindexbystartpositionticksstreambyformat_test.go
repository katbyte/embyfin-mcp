...
func TestOperationGetItemsByIdByMediaSourceIdSubtitlesByIndexByStartPositionTicksStreamByFormat(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "text/plain", "file bytes")
	result, err := c.GetItemsByIdByMediaSourceIdSubtitlesByIndexByStartPositionTicksStreamByFormat(t.Context(), "p/id", "p/mediaSourceId", 7, 7, "p/format", GetItemsByIdByMediaSourceIdSubtitlesByIndexByStartPositionTicksStreamByFormatOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2Fid/p%2FmediaSourceId/Subtitles/7/7/Stream.p%2Fformat")
