...
func TestOperationGetConnectExchange(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetConnectExchange(t.Context(), GetConnectExchangeOperationOptions{
		ConnectUserId: "v-ConnectUserId",
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Connect/Exchange")
	expectQuery(t, r, "ConnectUserId", "v-ConnectUserId")
