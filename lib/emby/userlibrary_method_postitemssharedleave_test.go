...
func TestOperationPostItemsSharedLeave(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostItemsSharedLeave(t.Context(), UserLibraryLeaveSharedItems{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/Shared/Leave")
