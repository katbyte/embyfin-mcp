...
func TestOperationPostDlnaProfilesById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDlnaProfilesById(t.Context(), "p/id", DlnaProfilesDlnaProfile{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Dlna/Profiles/p%2Fid")
