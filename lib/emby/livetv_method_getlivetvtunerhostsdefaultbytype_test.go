...
func TestOperationGetLiveTvTunerHostsDefaultByType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetLiveTvTunerHostsDefaultByType(t.Context(), "p/typeParam")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/TunerHosts/Default/p%2FtypeParam")
