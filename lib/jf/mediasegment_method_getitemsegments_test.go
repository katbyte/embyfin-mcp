...
func TestOperationGetItemSegments(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetItemSegments(t.Context(), "p/itemId", GetItemSegmentsOperationOptions{
		IncludeSegmentTypes: []MediaSegmentType{MediaSegmentTypeUnknown, MediaSegmentTypeIntro},
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/MediaSegments/p%2FitemId")
	expectQuery(t, r, "includeSegmentTypes", "Unknown", "Intro")
