...
func TestOperationDiscoverTuners(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.DiscoverTuners(t.Context(), DiscoverTunersOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/Tuners/Discover")
