...
func TestOperationPostUserImage(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.PostUserImage(t.Context(), strings.NewReader("raw bytes"), "image/png", PostUserImageOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/UserImage")
