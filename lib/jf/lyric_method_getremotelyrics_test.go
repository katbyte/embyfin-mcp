...
func TestOperationGetRemoteLyrics(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetRemoteLyrics(t.Context(), "p/lyricId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Providers/Lyrics/p%2FlyricId")
