...
func TestOperationUpdateItemUserData(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.UpdateItemUserData(t.Context(), "p/itemId", UpdateUserItemDataDto{}, UpdateItemUserDataOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/UserItems/p%2FitemId/UserData")
