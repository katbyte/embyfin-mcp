...
func TestOperationMarkPlayedItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MarkPlayedItem(t.Context(), "p/itemId", MarkPlayedItemOperationOptions{
		UserId:     "v-UserId",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/UserPlayedItems/p%2FitemId")
