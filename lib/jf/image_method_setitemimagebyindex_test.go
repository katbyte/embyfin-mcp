...
func TestOperationSetItemImageByIndex(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SetItemImageByIndex(t.Context(), "p/itemId", ImageTypePrimary, 7, strings.NewReader("raw bytes"), "image/png")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
