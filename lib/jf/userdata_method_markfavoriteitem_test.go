...
func TestOperationMarkFavoriteItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MarkFavoriteItem(t.Context(), "p/itemId", MarkFavoriteItemOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/UserFavoriteItems/p%2FitemId")
