...
func TestOperationGetQuickConnectState(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetQuickConnectState(t.Context(), GetQuickConnectStateOperationOptions{
		Secret: "v-Secret",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/QuickConnect/Connect")
	expectQuery(t, r, "secret", "v-Secret")
