...
func TestOperationUpdatePlaylist(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdatePlaylist(t.Context(), "p/playlistId", UpdatePlaylistDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Playlists/p%2FplaylistId")
