...
func TestOperationGetMusicGenre(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetMusicGenre(t.Context(), "p/genreName", GetMusicGenreOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/MusicGenres/p%2FgenreName")
