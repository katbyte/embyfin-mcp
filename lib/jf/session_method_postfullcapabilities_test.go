...
func TestOperationPostFullCapabilities(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.PostFullCapabilities(t.Context(), ClientCapabilitiesDto{}, PostFullCapabilitiesOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/Capabilities/Full")
