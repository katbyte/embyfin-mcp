...
func TestOperationAuthorizeQuickConnect(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "true")
	result, err := c.AuthorizeQuickConnect(t.Context(), AuthorizeQuickConnectOperationOptions{
		Code:   "v-Code",
		UserId: "v-UserId",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/QuickConnect/Authorize")
	expectQuery(t, r, "code", "v-Code")
