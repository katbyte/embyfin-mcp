...
func TestOperationGetInstantMixFromMusicGenreByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetInstantMixFromMusicGenreByName(t.Context(), "p/name", GetInstantMixFromMusicGenreByNameOperationOptions{
		UserId:           "v-UserId",
		Limit:            7,
