...
func TestOperationUpdateUserItemRating(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.UpdateUserItemRating(t.Context(), "p/itemId", UpdateUserItemRatingOperationOptions{
		UserId: "v-UserId",
		Likes:  new(true),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/UserItems/p%2FitemId/Rating")
