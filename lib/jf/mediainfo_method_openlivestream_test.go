...
func TestOperationOpenLiveStream(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.OpenLiveStream(t.Context(), OpenLiveStreamDto{}, OpenLiveStreamOperationOptions{
		...
		AlwaysBurnInSubtitleWhenTranscoding: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	expectQuery(t, r, "alwaysBurnInSubtitleWhenTranscoding", "true")
