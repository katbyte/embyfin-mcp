...
func TestOperationSyncPlaySetIgnoreWait(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlaySetIgnoreWait(t.Context(), IgnoreWaitRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/SyncPlay/SetIgnoreWait")
