...
func TestOperationStartRestoreBackup(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.StartRestoreBackup(t.Context(), BackupRestoreRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Backup/Restore")
