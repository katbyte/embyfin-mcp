...
func TestOperationReportPlaybackStopped(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.ReportPlaybackStopped(t.Context(), PlaybackStopInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/Playing/Stopped")
