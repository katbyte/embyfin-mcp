...
func TestOperationSetRemoteAccess(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SetRemoteAccess(t.Context(), StartupRemoteAccessDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Startup/RemoteAccess")
