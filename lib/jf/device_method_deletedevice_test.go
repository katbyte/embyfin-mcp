...
func TestOperationDeleteDevice(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.DeleteDevice(t.Context(), DeleteDeviceOperationOptions{
		Id: []string{"a", "b"},
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Devices")
	expectQuery(t, r, "id", "a", "b")
