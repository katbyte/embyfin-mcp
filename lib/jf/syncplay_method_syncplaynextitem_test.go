...
func TestOperationSyncPlayNextItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlayNextItem(t.Context(), NextItemRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/SyncPlay/NextItem")
