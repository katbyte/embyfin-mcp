...
func TestOperationSendFullGeneralCommand(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SendFullGeneralCommand(t.Context(), "p/sessionId", GeneralCommand{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Sessions/p%2FsessionId/Command")
