...
func TestOperationGetPlaylistUser(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetPlaylistUser(t.Context(), "p/playlistId", "p/userId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Playlists/p%2FplaylistId/Users/p%2FuserId")
