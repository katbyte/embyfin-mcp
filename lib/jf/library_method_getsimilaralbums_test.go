...
func TestOperationGetSimilarAlbums(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSimilarAlbums(t.Context(), "p/itemId", GetSimilarAlbumsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Albums/p%2FitemId/Similar")
