...
func TestOperationGetDisplayPreferences(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetDisplayPreferences(t.Context(), "p/displayPreferencesId", GetDisplayPreferencesOperationOptions{
		UserId: "v-UserId",
		Client: "v-Client",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/DisplayPreferences/p%2FdisplayPreferencesId")
