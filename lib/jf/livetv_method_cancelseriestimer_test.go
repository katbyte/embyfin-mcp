...
func TestOperationCancelSeriesTimer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.CancelSeriesTimer(t.Context(), "p/timerId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/LiveTv/SeriesTimers/p%2FtimerId")
