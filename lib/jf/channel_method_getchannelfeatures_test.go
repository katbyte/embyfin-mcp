...
func TestOperationGetChannelFeatures(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetChannelFeatures(t.Context(), "p/channelId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Channels/p%2FchannelId/Features")
