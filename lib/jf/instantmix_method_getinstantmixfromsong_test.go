...
func TestOperationGetInstantMixFromSong(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetInstantMixFromSong(t.Context(), "p/itemId", GetInstantMixFromSongOperationOptions{
		UserId:           "v-UserId",
		Limit:            new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Songs/p%2FitemId/InstantMix")
