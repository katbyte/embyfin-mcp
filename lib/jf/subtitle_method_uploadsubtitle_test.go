...
func TestOperationUploadSubtitle(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UploadSubtitle(t.Context(), "p/itemId", UploadSubtitleDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Videos/p%2FitemId/Subtitles")
