...
func TestOperationGetParentPath(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "\"s\"")
	result, err := c.GetParentPath(t.Context(), GetParentPathOperationOptions{
		Path: "v-Path",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Environment/ParentPath")
