...
func TestOperationAddMediaPath(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.AddMediaPath(t.Context(), MediaPathDto{}, AddMediaPathOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Library/VirtualFolders/Paths")
