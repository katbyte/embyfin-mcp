...
func TestOperationGetUpcomingEpisodesComplete(t *testing.T) {
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetUpcomingEpisodesOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetUpcomingEpisodesOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetUpcomingEpisodesOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetUpcomingEpisodesOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "startIndex", "{}", tc.total, tc.pages)
			result, err := c.GetUpcomingEpisodesComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
