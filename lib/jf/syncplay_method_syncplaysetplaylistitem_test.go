...
func TestOperationSyncPlaySetPlaylistItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlaySetPlaylistItem(t.Context(), SetPlaylistItemRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/SyncPlay/SetPlaylistItem")
