...
func TestOperationUpdateTimer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateTimer(t.Context(), "p/timerId", TimerInfoDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/LiveTv/Timers/p%2FtimerId")
