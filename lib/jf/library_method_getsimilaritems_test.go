...
func TestOperationGetSimilarItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSimilarItems(t.Context(), "p/itemId", GetSimilarItemsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2FitemId/Similar")
