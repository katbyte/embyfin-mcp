...
func TestOperationUpdateUserPolicy(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateUserPolicy(t.Context(), "p/userId", UserPolicy{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/p%2FuserId/Policy")
