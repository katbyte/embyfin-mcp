...
func TestOperationEnablePlugin(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.EnablePlugin(t.Context(), "p/pluginId", "p/version")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Plugins/p%2FpluginId/p%2Fversion/Enable")
