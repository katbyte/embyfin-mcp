...
func TestOperationGetChannelItemsComplete(t *testing.T) {
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetChannelItemsOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetChannelItemsOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetChannelItemsOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetChannelItemsOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "startIndex", "{}", tc.total, tc.pages)
			result, err := c.GetChannelItemsComplete(t.Context(), "p/channelId", tc.options)
			if err != nil {
				t.Fatal(err)
			}
