...
func TestOperationUpdateBrandingConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateBrandingConfiguration(t.Context(), BrandingOptionsDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/System/Configuration/Branding")
