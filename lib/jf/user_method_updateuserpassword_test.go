...
func TestOperationUpdateUserPassword(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateUserPassword(t.Context(), UpdateUserPassword{}, UpdateUserPasswordOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Users/Password")
