...
func TestOperationUpdateTask(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateTask(t.Context(), "p/taskId", []TaskTriggerInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/ScheduledTasks/p%2FtaskId/Triggers")
