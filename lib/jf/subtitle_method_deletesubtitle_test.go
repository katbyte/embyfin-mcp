...
func TestOperationDeleteSubtitle(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.DeleteSubtitle(t.Context(), "p/itemId", 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Videos/p%2FitemId/Subtitles/7")
