...
func TestOperationDownloadRemoteLyrics(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.DownloadRemoteLyrics(t.Context(), "p/itemId", "p/lyricId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Audio/p%2FitemId/RemoteSearch/Lyrics/p%2FlyricId")
