...
func TestOperationUpdateConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateConfiguration(t.Context(), ServerConfiguration{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/System/Configuration")
