...
func TestOperationCreateCollection(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.CreateCollection(t.Context(), CreateCollectionOperationOptions{
		Name:     "v-Name",
		Ids:      []string{"a", "b"},
		ParentId: "v-ParentId",
		IsLocked: new(true),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Collections")
