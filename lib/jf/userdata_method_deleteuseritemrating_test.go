...
func TestOperationDeleteUserItemRating(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.DeleteUserItemRating(t.Context(), "p/itemId", DeleteUserItemRatingOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/UserItems/p%2FitemId/Rating")
