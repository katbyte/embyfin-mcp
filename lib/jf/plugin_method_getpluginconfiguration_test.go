...
func TestOperationGetPluginConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetPluginConfiguration(t.Context(), "p/pluginId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Plugins/p%2FpluginId/Configuration")
