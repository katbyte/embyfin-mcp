...
func TestOperationSyncPlaySetShuffleMode(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlaySetShuffleMode(t.Context(), SetShuffleModeRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/SyncPlay/SetShuffleMode")
