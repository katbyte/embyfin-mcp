...
func TestOperationSyncPlayBuffering(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlayBuffering(t.Context(), BufferRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/SyncPlay/Buffering")
