...
func TestOperationGetResumeItems(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetResumeItems(t.Context(), GetResumeItemsOperationOptions{
		UserId:                 "v-UserId",
		StartIndex:             new(7),
		Limit:                  new(7),
		...
		ExcludeActiveSessions:  new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/UserItems/Resume")
	...
	expectQuery(t, r, "excludeActiveSessions", "true")
...
func TestOperationGetResumeItemsComplete(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		name         string
		total, pages int
		options      GetResumeItemsOperationOptions
		items        int
		starts       []string
	}{
		{"total reached", 2, 9, GetResumeItemsOperationOptions{Limit: new(1)}, 2, []string{"", "1"}},
		{"empty page", 0, 2, GetResumeItemsOperationOptions{Limit: new(1)}, 2, []string{"", "1", "2"}},
		{"short page", 0, 9, GetResumeItemsOperationOptions{Limit: new(2)}, 1, []string{""}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			c, starts := pagedServer(t, "startIndex", "{}", tc.total, tc.pages)
			result, err := c.GetResumeItemsComplete(t.Context(), tc.options)
			if err != nil {
				t.Fatal(err)
			}
