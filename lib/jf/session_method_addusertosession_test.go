...
func TestOperationAddUserToSession(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.AddUserToSession(t.Context(), "p/sessionId", "p/userId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/p%2FsessionId/User/p%2FuserId")
