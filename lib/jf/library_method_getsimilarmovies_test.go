...
func TestOperationGetSimilarMovies(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSimilarMovies(t.Context(), "p/itemId", GetSimilarMoviesOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Movies/p%2FitemId/Similar")
