...
func TestOperationUpdateSeriesTimer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateSeriesTimer(t.Context(), "p/timerId", SeriesTimerInfoDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/LiveTv/SeriesTimers/p%2FtimerId")
