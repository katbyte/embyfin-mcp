...
func TestOperationUninstallPlugin(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UninstallPlugin(t.Context(), "p/pluginId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Plugins/p%2FpluginId")
