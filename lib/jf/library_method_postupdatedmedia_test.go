...
func TestOperationPostUpdatedMedia(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.PostUpdatedMedia(t.Context(), MediaUpdateInfoDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Library/Media/Updated")
