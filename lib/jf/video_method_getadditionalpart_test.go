...
func TestOperationGetAdditionalPart(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetAdditionalPart(t.Context(), "p/itemId", GetAdditionalPartOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Videos/p%2FitemId/AdditionalParts")
