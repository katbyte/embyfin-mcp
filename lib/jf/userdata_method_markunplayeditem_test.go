...
func TestOperationMarkUnplayedItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MarkUnplayedItem(t.Context(), "p/itemId", MarkUnplayedItemOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/UserPlayedItems/p%2FitemId")
