...
func TestOperationUpdateNamedConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateNamedConfiguration(t.Context(), "p/key", json.RawMessage("{}"))
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/System/Configuration/p%2Fkey")
