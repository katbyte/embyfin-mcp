...
func TestOperationReportPlaybackStart(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.ReportPlaybackStart(t.Context(), PlaybackStartInfo{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Sessions/Playing")
