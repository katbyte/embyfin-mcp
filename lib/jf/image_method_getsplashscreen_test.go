...
func TestOperationGetSplashscreen(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetSplashscreen(t.Context(), GetSplashscreenOperationOptions{
		Tag:    "v-Tag",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Branding/Splashscreen")
