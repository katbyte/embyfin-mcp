...
func TestOperationSearchRemoteSubtitles(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.SearchRemoteSubtitles(t.Context(), "p/itemId", "p/language", SearchRemoteSubtitlesOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2FitemId/RemoteSearch/Subtitles/p%2Flanguage")
