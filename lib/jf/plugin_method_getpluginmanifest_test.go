...
func TestOperationGetPluginManifest(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.GetPluginManifest(t.Context(), "p/pluginId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Plugins/p%2FpluginId/Manifest")
