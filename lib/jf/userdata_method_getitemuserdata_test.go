...
func TestOperationGetItemUserData(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetItemUserData(t.Context(), "p/itemId", GetItemUserDataOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/UserItems/p%2FitemId/UserData")
