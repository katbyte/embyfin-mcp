...
func TestOperationReportViewing(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.ReportViewing(t.Context(), ReportViewingOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/Viewing")
	expectQuery(t, r, "sessionId", "v-SessionId")
