...
func TestOperationGetInstantMixFromItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetInstantMixFromItem(t.Context(), "p/itemId", GetInstantMixFromItemOperationOptions{
		UserId:           "v-UserId",
		Limit:            new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2FitemId/InstantMix")
