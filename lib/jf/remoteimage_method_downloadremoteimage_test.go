...
func TestOperationDownloadRemoteImage(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.DownloadRemoteImage(t.Context(), "p/itemId", DownloadRemoteImageOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/p%2FitemId/RemoteImages/Download")
