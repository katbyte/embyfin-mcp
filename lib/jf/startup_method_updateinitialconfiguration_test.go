...
func TestOperationUpdateInitialConfiguration(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateInitialConfiguration(t.Context(), StartupConfigurationDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Startup/Configuration")
