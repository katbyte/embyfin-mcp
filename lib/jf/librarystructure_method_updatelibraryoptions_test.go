...
func TestOperationUpdateLibraryOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateLibraryOptions(t.Context(), UpdateLibraryOptionsDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Library/VirtualFolders/LibraryOptions")
