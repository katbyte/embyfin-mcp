...
func TestOperationUnmarkFavoriteItem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.UnmarkFavoriteItem(t.Context(), "p/itemId", UnmarkFavoriteItemOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/UserFavoriteItems/p%2FitemId")
