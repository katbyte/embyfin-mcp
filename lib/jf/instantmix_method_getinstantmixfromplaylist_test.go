...
func TestOperationGetInstantMixFromPlaylist(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetInstantMixFromPlaylist(t.Context(), "p/itemId", GetInstantMixFromPlaylistOperationOptions{
		UserId:           "v-UserId",
		Limit:            new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Playlists/p%2FitemId/InstantMix")
