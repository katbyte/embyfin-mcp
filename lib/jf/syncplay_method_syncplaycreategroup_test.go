...
func TestOperationSyncPlayCreateGroup(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.SyncPlayCreateGroup(t.Context(), NewGroupRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/SyncPlay/New")
