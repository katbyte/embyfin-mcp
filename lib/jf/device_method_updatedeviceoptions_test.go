...
func TestOperationUpdateDeviceOptions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdateDeviceOptions(t.Context(), DeviceOptionsDto{}, UpdateDeviceOptionsOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/Devices/Options")
