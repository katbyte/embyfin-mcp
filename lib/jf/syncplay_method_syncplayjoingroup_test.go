...
func TestOperationSyncPlayJoinGroup(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SyncPlayJoinGroup(t.Context(), JoinGroupRequestDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/SyncPlay/Join")
