...
func TestOperationGetDashboardConfigurationPage(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/x-javascript", "file bytes")
	result, err := c.GetDashboardConfigurationPage(t.Context(), GetDashboardConfigurationPageOperationOptions{
		Name: "v-Name",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/web/ConfigurationPage")
