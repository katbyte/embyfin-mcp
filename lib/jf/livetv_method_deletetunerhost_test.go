...
func TestOperationDeleteTunerHost(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.DeleteTunerHost(t.Context(), DeleteTunerHostOperationOptions{
		Id: "v-Id",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/LiveTv/TunerHosts")
