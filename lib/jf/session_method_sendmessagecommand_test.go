...
func TestOperationSendMessageCommand(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.SendMessageCommand(t.Context(), "p/sessionId", MessageCommand{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Sessions/p%2FsessionId/Message")
