...
func TestOperationCancelPackageInstallation(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.CancelPackageInstallation(t.Context(), "p/packageId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/Packages/Installing/p%2FpackageId")
