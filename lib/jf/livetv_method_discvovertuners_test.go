...
func TestOperationDiscvoverTuners(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.DiscvoverTuners(t.Context(), DiscvoverTunersOperationOptions{
		NewDevicesOnly: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/LiveTv/Tuners/Discvover")
	expectQuery(t, r, "newDevicesOnly", "true")
