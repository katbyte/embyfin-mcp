...
func TestOperationGetTrickplayTileImage(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "image/png", "file bytes")
	result, err := c.GetTrickplayTileImage(t.Context(), "p/itemId", 7, 7, GetTrickplayTileImageOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Videos/p%2FitemId/Trickplay/7/7.jpg")
