...
func TestOperationGetMusicVideoRemoteSearchResults(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetMusicVideoRemoteSearchResults(t.Context(), MusicVideoInfoRemoteSearchQuery{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/RemoteSearch/MusicVideo")
