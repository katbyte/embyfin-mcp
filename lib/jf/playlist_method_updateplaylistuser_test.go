...
func TestOperationUpdatePlaylistUser(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 204, "", "")
	result, err := c.UpdatePlaylistUser(t.Context(), "p/playlistId", "p/userId", UpdatePlaylistUserDto{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Playlists/p%2FplaylistId/Users/p%2FuserId")
