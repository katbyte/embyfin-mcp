...
func TestOperationGetConfigurationPages(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetConfigurationPages(t.Context(), GetConfigurationPagesOperationOptions{
		EnableInMainMenu: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/web/ConfigurationPages")
	expectQuery(t, r, "enableInMainMenu", "true")
