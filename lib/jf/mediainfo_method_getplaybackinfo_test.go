...
func TestOperationGetPlaybackInfo(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetPlaybackInfo(t.Context(), "p/itemId", GetPlaybackInfoOperationOptions{
		UserId: "v-UserId",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Items/p%2FitemId/PlaybackInfo")
