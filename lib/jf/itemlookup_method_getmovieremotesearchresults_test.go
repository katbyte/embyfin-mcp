...
func TestOperationGetMovieRemoteSearchResults(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetMovieRemoteSearchResults(t.Context(), MovieInfoRemoteSearchQuery{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/Items/RemoteSearch/Movie")
