...
func TestOperationGetQuickConnectEnabled(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "true")
	result, err := c.GetQuickConnectEnabled(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/QuickConnect/Enabled")
