...
func TestOperationSearchRemoteLyrics(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.SearchRemoteLyrics(t.Context(), "p/itemId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Audio/p%2FitemId/RemoteSearch/Lyrics")
