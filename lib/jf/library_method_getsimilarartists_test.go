...
func TestOperationGetSimilarArtists(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetSimilarArtists(t.Context(), "p/itemId", GetSimilarArtistsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/Artists/p%2FitemId/Similar")
