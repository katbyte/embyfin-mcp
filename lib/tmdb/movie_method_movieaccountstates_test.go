...
func TestOperationMovieAccountStates(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieAccountStates(t.Context(), 7, MovieAccountStatesOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/7/account_states")
