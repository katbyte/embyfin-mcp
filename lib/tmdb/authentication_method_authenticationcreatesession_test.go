...
func TestOperationAuthenticationCreateSession(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AuthenticationCreateSession(t.Context(), AuthenticationCreateSessionRequest{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/authentication/session/new")
