...
func TestOperationTvEpisodeGroupDetails(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvEpisodeGroupDetails(t.Context(), "p/tvEpisodeGroupId")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/episode_group/p%2FtvEpisodeGroupId")
