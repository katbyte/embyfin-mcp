...
func TestOperationListRemoveMovie(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.ListRemoveMovie(t.Context(), 7, ListRemoveMovieRequest{}, ListRemoveMovieOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/list/7/remove_item")
