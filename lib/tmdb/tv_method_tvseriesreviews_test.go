...
func TestOperationTvSeriesReviews(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvSeriesReviews(t.Context(), 7, TvSeriesReviewsOperationOptions{
		Language: "v-Language",
		Page:     7,
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/7/reviews")
