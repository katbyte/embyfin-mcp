...
func TestOperationGuestSessionRatedMovies(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GuestSessionRatedMovies(t.Context(), "p/guestSessionId", GuestSessionRatedMoviesOperationOptions{
		Language: "v-Language",
		Page:     new(7),
		SortBy:   "v-SortBy",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/guest_session/p%2FguestSessionId/rated/movies")
