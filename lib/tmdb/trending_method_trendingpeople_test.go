...
func TestOperationTrendingPeople(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TrendingPeople(t.Context(), "p/timeWindow", TrendingPeopleOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/trending/person/p%2FtimeWindow")
