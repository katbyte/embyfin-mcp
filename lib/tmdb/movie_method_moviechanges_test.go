...
func TestOperationMovieChanges(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieChanges(t.Context(), 7, MovieChangesOperationOptions{
		EndDate:   "v-EndDate",
		Page:      new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/7/changes")
