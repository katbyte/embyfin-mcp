...
func TestOperationWatchProvidersAvailableRegions(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.WatchProvidersAvailableRegions(t.Context(), WatchProvidersAvailableRegionsOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/watch/providers/regions")
