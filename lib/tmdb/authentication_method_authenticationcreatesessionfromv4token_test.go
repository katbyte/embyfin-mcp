...
func TestOperationAuthenticationCreateSessionFromV4Token(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AuthenticationCreateSessionFromV4Token(t.Context(), AuthenticationCreateSessionFromV4TokenRequest{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/authentication/session/convert/4")
