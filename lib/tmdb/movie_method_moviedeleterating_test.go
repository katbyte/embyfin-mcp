...
func TestOperationMovieDeleteRating(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieDeleteRating(t.Context(), 7, MovieDeleteRatingOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/3/movie/7/rating")
