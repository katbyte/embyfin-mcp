...
func TestOperationAuthenticationCreateSessionFromLogin(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AuthenticationCreateSessionFromLogin(t.Context(), AuthenticationCreateSessionFromLoginRequest{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/authentication/token/validate_with_login")
