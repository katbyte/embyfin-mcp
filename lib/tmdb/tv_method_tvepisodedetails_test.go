...
func TestOperationTvEpisodeDetails(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvEpisodeDetails(t.Context(), 7, 7, 7, TvEpisodeDetailsOperationOptions{
		AppendToResponse: "v-AppendToResponse",
		Language:         "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/7/season/7/episode/7")
	expectQuery(t, r, "append_to_response", "v-AppendToResponse")
