...
func TestOperationMovieAlternativeTitles(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieAlternativeTitles(t.Context(), 7, MovieAlternativeTitlesOperationOptions{
		Country: "v-Country",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/7/alternative_titles")
