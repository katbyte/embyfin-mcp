...
func TestOperationAuthenticationDeleteSession(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AuthenticationDeleteSession(t.Context(), AuthenticationDeleteSessionRequest{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	expectRequest(t, r, http.MethodDelete, "/3/authentication/session")
