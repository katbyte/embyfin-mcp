...
func TestOperationTvSeasonDetails(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvSeasonDetails(t.Context(), 7, 7, TvSeasonDetailsOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/7/season/7")
