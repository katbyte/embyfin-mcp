...
func TestOperationMovieNowPlayingList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieNowPlayingList(t.Context(), MovieNowPlayingListOperationOptions{
		Language: "v-Language",
		Page:     new(7),
		Region:   "v-Region",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/now_playing")
