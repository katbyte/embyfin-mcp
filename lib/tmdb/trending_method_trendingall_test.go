...
func TestOperationTrendingAll(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TrendingAll(t.Context(), "p/timeWindow", TrendingAllOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/trending/all/p%2FtimeWindow")
