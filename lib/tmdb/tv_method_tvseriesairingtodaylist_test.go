...
func TestOperationTvSeriesAiringTodayList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvSeriesAiringTodayList(t.Context(), TvSeriesAiringTodayListOperationOptions{
		Language: "v-Language",
		Page:     7,
		Timezone: "v-Timezone",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/airing_today")
