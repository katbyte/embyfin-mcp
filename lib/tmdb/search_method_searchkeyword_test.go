...
func TestOperationSearchKeyword(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.SearchKeyword(t.Context(), SearchKeywordOperationOptions{
		Query: "v-Query",
		Page:  7,
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/search/keyword")
