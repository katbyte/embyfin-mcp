...
func TestOperationMovieSimilar(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieSimilar(t.Context(), 7, MovieSimilarOperationOptions{
		Language: "v-Language",
		Page:     7,
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/7/similar")
