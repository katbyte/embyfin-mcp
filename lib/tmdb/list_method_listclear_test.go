...
func TestOperationListClear(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.ListClear(t.Context(), 7, ListClearOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/3/list/7/clear")
	expectQuery(t, r, "session_id", "v-SessionId")
	expectQuery(t, r, "confirm", "true")
