...
func TestOperationPersonPopularList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PersonPopularList(t.Context(), PersonPopularListOperationOptions{
		Language: "v-Language",
		Page:     new(7),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/person/popular")
