...
func TestOperationChangesPeopleList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.ChangesPeopleList(t.Context(), ChangesPeopleListOperationOptions{
		EndDate:   "v-EndDate",
		Page:      7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/person/changes")
