...
func TestOperationMoviePopularList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MoviePopularList(t.Context(), MoviePopularListOperationOptions{
		Language: "v-Language",
		Page:     new(7),
		Region:   "v-Region",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/popular")
