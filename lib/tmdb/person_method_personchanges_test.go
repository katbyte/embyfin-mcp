...
func TestOperationPersonChanges(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PersonChanges(t.Context(), 7, PersonChangesOperationOptions{
		EndDate:   "v-EndDate",
		Page:      new(7),
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/person/7/changes")
