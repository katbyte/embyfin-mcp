...
func TestOperationMovieAddRating(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieAddRating(t.Context(), 7, MovieAddRatingRequest{}, MovieAddRatingOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/movie/7/rating")
