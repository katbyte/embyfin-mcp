...
func TestOperationAccountWatchlistTv(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AccountWatchlistTv(t.Context(), 7, AccountWatchlistTvOperationOptions{
		Language:  "v-Language",
		Page:      7,
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/account/7/watchlist/tv")
