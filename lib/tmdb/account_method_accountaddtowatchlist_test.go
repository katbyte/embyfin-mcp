...
func TestOperationAccountAddToWatchlist(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.AccountAddToWatchlist(t.Context(), 7, AccountAddToWatchlistRequest{}, AccountAddToWatchlistOperationOptions{
	...
	r, body := s.only(t)
	expectRequest(t, r, http.MethodPost, "/3/account/7/watchlist")
