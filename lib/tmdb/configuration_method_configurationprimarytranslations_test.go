...
func TestOperationConfigurationPrimaryTranslations(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[\"s\"]")
	result, err := c.ConfigurationPrimaryTranslations(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/configuration/primary_translations")
