...
func TestOperationTrendingMovies(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TrendingMovies(t.Context(), "p/timeWindow", TrendingMoviesOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/trending/movie/p%2FtimeWindow")
