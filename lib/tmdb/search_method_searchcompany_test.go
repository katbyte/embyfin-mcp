...
func TestOperationSearchCompany(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.SearchCompany(t.Context(), SearchCompanyOperationOptions{
		Query: "v-Query",
		Page:  new(7),
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/search/company")
