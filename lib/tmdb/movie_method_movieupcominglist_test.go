...
func TestOperationMovieUpcomingList(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.MovieUpcomingList(t.Context(), MovieUpcomingListOperationOptions{
		Language: "v-Language",
		Page:     7,
		Region:   "v-Region",
	})
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/movie/upcoming")
