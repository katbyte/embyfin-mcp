...
func TestOperationTrendingTv(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TrendingTv(t.Context(), "p/timeWindow", TrendingTvOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/trending/tv/p%2FtimeWindow")
