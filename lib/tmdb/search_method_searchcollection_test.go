...
func TestOperationSearchCollection(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.SearchCollection(t.Context(), SearchCollectionOperationOptions{
		Query:        "v-Query",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/search/collection")
