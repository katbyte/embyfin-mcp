...
func TestOperationTvEpisodeImages(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.TvEpisodeImages(t.Context(), 7, 7, 7, TvEpisodeImagesOperationOptions{
		IncludeImageLanguage: "v-IncludeImageLanguage",
		Language:             "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/tv/7/season/7/episode/7/images")
	expectQuery(t, r, "include_image_language", "v-IncludeImageLanguage")
