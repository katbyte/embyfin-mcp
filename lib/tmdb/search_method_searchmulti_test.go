...
func TestOperationSearchMulti(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.SearchMulti(t.Context(), SearchMultiOperationOptions{
		Query:        "v-Query",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/search/multi")
