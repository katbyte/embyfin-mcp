...
func TestOperationPersonCombinedCredits(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PersonCombinedCredits(t.Context(), 7, PersonCombinedCreditsOperationOptions{
		Language: "v-Language",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/3/person/7/combined_credits")
