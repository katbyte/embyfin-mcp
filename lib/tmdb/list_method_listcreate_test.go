...
func TestOperationListCreate(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.ListCreate(t.Context(), ListCreateRequest{}, ListCreateOperationOptions{
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/3/list")
