...
func run(t *testing.T, args ...string) (string, error) {
	...
	// a developer's own ~/.embyfin-mcp must not leak into the run: with a
...
func TestMakeBuildsTheCommands(t *testing.T) {
	...
	out, err = run(t)
	if err != nil || !strings.Contains(out, "embyfin-mcp help") {
...
func TestVersionCommand(t *testing.T) {
	...
	if err != nil || !strings.HasPrefix(out, "embyfin-mcp ") {
...
func TestInfoNeedsConnectionParams(t *testing.T) {
	...
	_, err = run(t, "serve", "--server", "http://nas:8096")
	if err == nil || !strings.Contains(err.Error(), "token") {
		t.Errorf("serve with no token = %v, want a complaint about token", err)
	}
}

// `embyfin-mcp tools` needs no server, so it has to work with nothing
...
func TestToolsCommandToolsets(t *testing.T) {
	...
	if !strings.Contains(out, "item_delete") || strings.Contains(out, "delete tools are hidden") {
