...
- **`audit_provider`** replaces `audit_movie_ids` and `audit_runtime`'s movie pass: one request a film checks both that its ids agree with each other and exist at the provider and that the file's runtime is the provider's, where the two sweeps asked TMDB twice for every film. `provider` names which to ask, TMDB being the only one yet, and `checks` narrows to ids or runtime. `audit_runtime` is the episode pass alone. Paged with `offset` and `next_offset` like every list (were `start_index` and `next_start_index`).
- **`audit_missing_episodes` with `provider: true`** reads every series' whole run from the configured metadata providers (TMDB today), one request a series and paged, and reports the aired episodes the run lists past the files, which the gaps between files cannot see; a series no provider knows is listed under `unknown` with why.
- **Outputs named alike across the audits.** Every audit answers `items_scanned` and `total_findings` (were `series_scanned`, `total_groups`, `total_folders`, `total_orphans` on five of them), and `audit_duplicates` answers `groups` (was `duplicate_groups`).
- `audit_duplicate_titles` is `audit_duplicate_episodes` and `audit_duplicate_series_folders` is `audit_duplicate_series`: named by what is held twice rather than by the evidence.
- `golang.org/x/text` moved to 0.39.0, past CVE-2026-56852, which the binary scan flagged.
...
- A playlist move a scan's refresh writes back is sent once more on both servers: Emby accepts the move and then saves the playlist as it found it, and Jellyfin's re-read of the playlist file can restore the old tail beside the new one, which was reported as someone else's edit.
...
- **`audit_duplicate_episodes`**: one episode's content filed under two episode numbers, which neither other duplicate audit can see.
- **`audit_title_mismatch`**: episodes whose file name claims a different title from the one the server holds.
- **`audit_duplicate_series`**: shows held twice because two folder names differ only by spacing, case, an accent or punctuation.
...
- **`audit_movie_ids`** (since folded into `audit_provider`): films whose ids disagree, asked of TMDB: a TMDB id whose film carries a different IMDb id, a TMDB id TMDB no longer has, or an IMDb id that is a series or an episode. Needs `EMBYFIN_TMDB_TOKEN`.
