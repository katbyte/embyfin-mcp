...
| curation | `audit_all` + 15 audits, `quality_compare`, `item_identify` → `item_identify_apply`, `item_refresh`, `item_edit`, `item_batch_edit`, `metadata_rename`, `item_artwork` → `item_artwork_set`, `item_subtitle_search` → `item_subtitle_download` | "what is wrong, and fix it" |
